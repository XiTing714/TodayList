...
export default {
  ...
  data() {
    return {
      doneTask: this.$store.state.todos.filter(item => item.completed),
      editIndex: null
    }
  },
  computed: {
    isAllToggled() {
      return this.doneTask.every(item => item.completed == true)
    },
  },
  /* watch: {
    activeTask: {
  ...
  methods: {
    // 全选
    handleToglleAll() {
      this.$store.commit('ToggleAllIndone')
    },
    // 单个项目勾选
    itemCompleted(index) {
      this.$store.commit('itemCompletedIndone', index)
    },
    // 删除单个项目
    handleDelete(index) {
      this.$store.commit('itemDeleteIndone', index)
    },
    // 双击进入编辑状态
    getEdit(index) {
      /* console.log('已双击'); */
    ...
    saveEdit(index, e) {
      ...
      this.$store.commit('saveEditIndone', {
        index,
        editText
      })
      this.editIndex = null

<template>
  <section class="main">
    ...
    <ul class="todo-list">
      <li v-for="(item, index) in allTask" :key="(item, index)" 
...
export default {
  ...
  data() {
    return {
      allTask: this.$store.state.todos,
      editIndex: null
    }
  },
  computed: {
    isAllToggled() {
      return this.allTask.every(item => item.completed == true)
    },
  },
  methods: {
    // 全选
    handleToglleAll() {
      this.$store.commit('ToggleAllInall')
    },
    // 单个项目勾选
    itemCompleted(index) {
      this.$store.commit('itemCompletedInall', index)
    },
    // 删除单个项目
    handleDelete(index) {
      this.$store.commit('itemDeleteInall', index)
    },
    // 双击进入编辑状态
    getEdit(index) {
      /* console.log('已双击'); */
    ...
    saveEdit(index, e) {
      ...
      this.$store.commit('saveEditInall', {
        index,
        editText
      })
      this.editIndex = null
